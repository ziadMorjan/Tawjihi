...
### [ميزة 7] اشتراكات الطلاب (My Enrollments Feature)

تمت تغطية هذه الميزة بالكامل عبر ملف اختبار يحتوي على 5 فحوصات ناجحة.

#### أ. خريطة ملفات اختبار الـ Enrollments:

* **اختبارات الـ Hooks:**
  * `src/features/enrollments/hooks/__tests__/useMyEnrollments.test.js` (التحقق من حالة اشتراك المستخدم الحالي في كورس معين، جلب الاشتراكات بنجاح، والتعامل مع عدم تسجيل الدخول).

#### ب. ملاحظات بيئة الاختبار الخاصة بالميزة:

* **محاكاة الـ useAuth للتأكد من الهوية:** يعتمد الهوك بشكل كامل على وجود معرف مستخدم نشط `user._id`؛ وقمنا بمحاكاة `useAuth` للتحقق من إيقاف جلب البيانات في حال عدم تسجيل الدخول.
* **فحص منطق التحقق من الاشتراك (`isEnrolled`):** تدعم الدالة التحقق سواء كانت العلاقة مخزنة كـ ID نصي أو ككائن مفصل يحتوي على `_id` داخلي، وتأكدنا من تغطية الحالتين بدقة وتفادي القيم غير المعرفة.

---

### [ميزة 8] المعلمون وتقييماتهم (Teachers Feature)

تمت تغطية هذه الميزة بالكامل عبر 3 ملفات اختبار تحتوي على 18 فحصاً ناجحاً.

#### أ. خريطة ملفات اختبار الـ Teachers:

* **اختبارات الـ Hooks:**
  * `src/features/teachers/hooks/__tests__/useTeachers.test.js` (جلب قائمة المعلمين والـ Pagination والتعامل مع النتائج الفارغة أو الأخطاء).
  * `src/features/teachers/hooks/__tests__/useTeacher.test.js` (جلب تفاصيل معلم معين عبر معرفه الشخصي وجلب قائمة الكورسات المرتبطة بالمعلم).
  * `src/features/teachers/hooks/__tests__/useTeacherReviews.test.js` (جلب تقييمات المعلم، إضافة تقييم جديد مع إبطال كاش المعلم والتقييمات، وحذف تقييم تفاؤلياً مع التراجع التلقائي).

#### ب. ملاحظات بيئة الاختبار الخاصة بالميزة:

* **منع التنظيف التلقائي للكاش (gcTime):** تم استخدام تقنية تسجيل الـ active observer عبر استدعاء هوك `useTeacherReviews` داخل فحوصات الحذف لمنع React Query من مسح الكاش فوراً بسبب قيمة `gcTime: 0`.
* **محاكاة البطء للاختبار التفاؤلي:** نستخدم وعوداً معلقة (unresolved promises) في محاكاة الحذف لفحص دقة التحديث التفاؤلي على الكاش قبل ردود السيرفر.

---

### [ميزة 9] البحث الموازي (Parallel Search Feature)

تمت تغطية هذه الميزة بالكامل عبر ملف اختبار يحتوي على 5 فحوصات ناجحة.

#### أ. خريطة ملفات اختبار الـ Search:

* **اختبارات الـ Hooks:**
  * `src/features/search/__tests__/useSearch.test.js` (التحقق من صحة الكلمة الدلالية، تشغيل استفسارين للبحث عن الكورسات والمعلمين بالتوازي، وتجميع النتائج وإدارة الأخطاء).

#### ب. ملاحظات بيئة الاختبار الخاصة بالميزة:

* **تشغيل الاستعلامات بالتوازي (Parallel Queries):** نتحقق من استدعاء دالتي `coursesApi.getAll` و `teachersApi.getAll` بشكل مستقل ومتوازٍ وإدارة حالة التحميل المشتركة (`isLoading` / `isFetching`) بنجاح.
* **التحقق من الكلمة الدلالية قبل الإرسال (Falsy Keywords):** نختبر إيقاف البحث التلقائي إذا كان المدخل فارغاً أو يحتوي على مسافات فارغة فقط لتقليل الضغط على السيرفر.

---

### [ميزة 10] الوضع الداكن وصفحة الخطأ (Theme & NotFound Feature)

تمت تغطية هذه الميزة بالكامل عبر ملفي اختبار يحتويان على 8 فحوصات ناجحة.

#### أ. خريطة ملفات اختبار الـ Theme & NotFound:

* **اختبارات الـ Theme Context:**
  * `src/features/theme/__tests__/ThemeContext.test.jsx` (التحقق من القيمة الافتراضية، وقراءتها من الـ localStorage، وتغيير الوضع مع تحديث السمة `data-theme` على الصفحة، والتحقق من رمي خطأ عند الاستخدام خارج الـ Provider).

* **اختبارات الـ NotFound Page:**
  * `src/pages/NotFound/__tests__/NotFound.test.jsx` (فحص عرض الرقم 404، والـ Terminal التشخيصي، وزر العودة والذهاب للرئيسية، والروابط السريعة الموجهة للمسارات الصحيحة).

#### ب. ملاحظات بيئة الاختبار الخاصة بالميزة:

* **التعامل مع الـ LocalStorage الافتراضية لـ JSDOM:** بيئة اختبار JSDOM تأتي بمحاكاة كاملة للـ LocalStorage؛ لذا تعاملنا معها مباشرة بدلاً من الموك المعقد لتسهيل الاختبار وزيادة الموثوقية.
* **محاكاة فريم Framer Motion في الاختبارات:** تم عمل Mock بسيط لمكونات مكتبة الحركة `framer-motion` لمنع حدوث مشاكل في أنماط التحريك والتحقق من المخرجات النصية الثابتة للمكونات.

---

### [ميزة 11] الميزات القادمة (مساحة مخصصة للإضافة)
*(قم بإضافة توثيق ميزات الإشعارات أو الميزات الأخرى هنا فور كتابة اختبارات لها لتتبع نفس الأسلوب).*
